...
n = 6
# book_3pages plus a pendant edge {0,5} to break the 0<->1 symmetry
...
for q in [2,3]:
    ...
    diff = chi_D - chi_Ddel
    print(f"\n=== q={q} ===  diff = {diff.factor()}   -(q-1)^(|F|-1) = {-(q-1)**2}")
    for e in private_edges:
        ...
        print(f"  e={sorted(e)}: chi(Delta/e) = {chi_con.factor()}   [diff == -(q-1)^2*chi_con ? {ok}]")
